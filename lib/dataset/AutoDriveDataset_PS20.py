...
from ..utils.augmentations import letterbox_PS20

class AutoDriveDataset_PS20(Dataset):
    """
    ...
    def __init__(self, cfg, is_train, inputsize=640, transform=None):
        ...
        if is_train:
            ...
            indicator = cfg.DATASET.TEST_SET + "/all"
        # self.img_root = os.path.join(img_root,indicator)
        self.img_root = img_root/ indicator
        self.img_root = list(self.img_root.iterdir())
    ...
    def __getitem__(self, idx):
        ...
        seg_label = cv2.imread(data["image"], 0)
        # if self.cfg.num_seg_class == 3:
        #     seg_label = cv2.imread(data["mask"])
        # else:
        #     seg_label = cv2.imread(data["mask"], 0)
        lane_label = cv2.imread(data["image"], 0)
        ...
        # img, ratio, pad = letterbox_PS20(img, resized_shape, auto=True, scaleup=self.is_train)
        ...
        #     _, x, y, hi, hi = bbox
        #     draw.rectangle([x*w - 0.5*hi*w, y*w  - 0.5*hi*w, x*w  + 0.5*hi*w , y*w  + 0.5*hi*w],outline=(255, 255, 0))
        # img_pil.show()
        ...
        # for bbox in labels:
        #     _,xi, yi, xj, yj = bbox
        #     draw.rectangle([xi, yi, xj , yj],outline=(255, 255, 0))
        # img_pil.show()
        ...
        # for bbox in labels:
        #     _,xi, yi, hi, wi = bbox
        #     draw.rectangle([(xi-0.5*hi)*w, (yi-0.5*wi)*w, (xi+0.5*hi)*w, (yi+0.5*wi)*w],outline=(255, 255, 0))
        # img_pil.show()
        ...
        if self.cfg.num_seg_class == 3:
            ...
            _,seg1 = cv2.threshold(seg_label,1,255,cv2.THRESH_BINARY)#大于1 就改成255
