...
def main():
    ...
    # print(cfg.GPUS)
    # model = torch.nn.DataParallel(model, device_ids=cfg.GPUS)
    # det = model.module.model[model.module.detector_index]
    # assign model params
    model.gr = 1.0
    model.nc = 1
    ...
    train_loader = DataLoaderX(#数据后台预先读取下一batch，让训练速度加快
        ...
        collate_fn=dataset.AutoDriveDataset_PS20.collate_fn
    )
    ...
    if rank in [-1, 0]:
        if cfg.NEED_AUTOANCHOR:
            ...
            if is_parallel(model):
                det = model.model[model.module.detector_index]
            else:
                det = model.model[model.detector_index]
            # det = model.model[model.module.detector_index] if is_parallel(model) \
    ...
    for epoch in range(begin_epoch+1, cfg.TRAIN.END_EPOCH+1):
        ...
        if (epoch % (5*cfg.TRAIN.VAL_FREQ) == 0 or epoch == cfg.TRAIN.END_EPOCH) and rank in [-1, 0]:
